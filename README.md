...
**Download PNG** for a 2480 × 3508 file — A4 at 300 DPI, saved as
...
whenever the slug does not read back as the seed (`Mixed Case` on `ink`
downloads as `noise-poster-mixed-case-928bb694-ink.png`). The picture is five
octaves of value noise flattened into flat tone bands: the seed picks a ceiling
of 5 to 8 and the sheet paints at most that many — across all 57,600 Shuffle
seeds the 2480 × 3508 export paints 4 to 8, and 4 only 13 times, while a small
preview can quantise a few more down to 4. The seed decides everything but the
palette, is cut to 64 code points (one 👍🏽 is two), and is printed small in the
bottom margin with runs of ASCII spaces and control characters collapsed and the
ends trimmed — so a sheet reproduces itself unless its seed had odd spacing or
control characters. Only the paper grain is drawn per device pixel, so the
preview is the export's composition but not its pixels. The address bar holds
`#s=<seed>&p=<palette>`, seed percent-encoded (`#s=north%20light&p=ink` on a
cold load), so sending the link sends the poster; a broken or half-typed hash
